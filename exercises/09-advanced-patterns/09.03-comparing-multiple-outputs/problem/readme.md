...
Both stream calls are being passed exactly the same messages, but we're using two different models - one using `gemini-2.5-flash-lite` and the other using `gemini-2.5-flash-lite`.
